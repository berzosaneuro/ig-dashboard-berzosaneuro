...
// matching the dashboard's original CROSS shape (one row per IG/FB pair).
function buildCrossPlatform(igPosts, fbPosts) {
  ...
  for (const fb of fbPosts) {
    ...
    if (!d) continue;
    const matches = igPosts.filter((ig) => dateOf(ig.timestamp) === d);
    for (const ig of matches) {
      rows.push({
        date: d,
...
module.exports = { percentileRanks, scorePosts, groupIntoWeeks, isoWeekLabel, buildCrossPlatform };
